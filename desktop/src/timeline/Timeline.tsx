// The Activity Timeline surface — desktop port. Owns the active type chip +
// search query (seeded from the loader so a scenario can capture a filtered
// view), derives the chip set from the data present, filters the day groups, and
// renders the filters + days. Two quiet states: nothing seeded (day-one) and
// nothing matching the current filter.
//
// This first port covers the read-only feed. Clicking a row to open its detail
// is deferred until the desktop in-app router lands (see TimelineEntry).
