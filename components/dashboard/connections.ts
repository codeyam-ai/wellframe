// Catalog + pure helpers for provider connections. Client-safe (NO prisma / no
// server imports) so the Connections panel can be a client component. The
// catalog of *available* providers lives here as static config; which ones are
// actually connected lives in the ProviderConnection table and arrives as
// ConnectionRow[] that we merge into view-model rows.
...
// The stored connection row (mirrors the Prisma ProviderConnection model, but
// declared here so client components don't transitively import @prisma/client).
...
// The raw ProviderConnection row as stored (the DB adds a numeric id; the string
// columns aren't yet narrowed to their unions). Declared here so the mapper stays
// client-safe and unit-testable without importing @prisma/client.
...
// Narrow a raw DB row to the typed ConnectionRow the UI consumes.
export function toConnectionRow(row: RawConnectionRow): ConnectionRow {
  return {
    providerId: row.providerId,
    kind: row.kind as ProviderKind,
    method: row.method as ConnectMethod,
    status: row.status as ConnectionStatus,
    detail: row.detail,
    endpoint: row.endpoint,
    isActiveCoach: row.isActiveCoach,
...
// deterministic stand-in for the real provider handshake, so it is unit-testable
// on its own and the Server Action only has to persist the result.
